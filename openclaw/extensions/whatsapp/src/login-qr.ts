...
export async function waitForWebLogin(
  ...
  while (true) {
    if (login.error) {
      const status =
        login.errorStatus ??
        (/status=515/i.test(String(login.error))
          ? 515
          : /status=408/i.test(String(login.error))
            ? 408
            : undefined);
      if (status === 401 || login.errorStatus === 401) {
        const message = WHATSAPP_LOGGED_OUT_QR_MESSAGE;
        await resetActiveLogin(account.accountId, message);
        runtime.log(danger(message));
        return { connected: false, message };
      }
      // 515 = post-scan pairing restart. Creds may still be flushing; do not
      // treat as a hard failure (outer clients must not force-clear auth).
      if (status === 515) {
        return {
          connected: false,
