class Api::V1::Accounts::InboxesController < Api::V1::Accounts::BaseController
  ...
  def set_agent_bot
    # OpenChat uses OpenClaw as the only agent bot for every channel.
    openclaw = Openchat::AttachOpenclawAgentService.openclaw_bot_for(Current.account)
    if openclaw
  ...
  def destroy
    if openclaw_managed?(@inbox)
      Openchat::SyncChannelTeardownJob.perform_later(account_id: Current.account.id, inbox_id: @inbox.id)
    end
    ::DeleteObjectJob.perform_later(@inbox, Current.user, request.ip) if @inbox.present?
    render status: :ok, json: { message: I18n.t('messages.inbox_deletetion_response') }
  end

  def openclaw_managed?(inbox)
    name = inbox.agent_bot&.name
    Openchat::AttachOpenclawAgentService.openclaw_bot_for(Current.account).present? &&
      (name == Openchat::ProvisionService::BOT_NAME || name.in?(Openchat::ProvisionService::LEGACY_BOT_NAMES))
  end
